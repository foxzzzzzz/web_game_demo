...
该脚本面向本项目的单台 Ubuntu 云服务器：安装 Git、Node.js、pnpm、Nginx，拉取指定 Git 仓库并构建，然后由 Nginx 直接托管 Vite 生成的静态文件。

## 首次部署

将整个项目（或至少 `deploy/ubuntu/` 目录）上传到服务器后，在项目根目录执行：

```bash
sudo bash deploy/ubuntu/deploy.sh
```

按提示输入 Git 仓库地址、分支或 tag、域名（可留空）、部署目录、应用名和端口。选择 HTTPS 时还会要求输入用于证书续期通知的邮箱。脚本会把配置保存为 `/etc/<应用名>.env`，权限为 `600`。
...
脚本会重新拉取、构建并发布；静态站点路径会切换到新的带时间戳版本目录。更新前如需改仓库、分支或域名，编辑该配置文件后再执行上述命令。
